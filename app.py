...
op = np.array([dict((("pt", ""), ("Confirmed",0), ("dept", ""), ("age", 0), ("sms", 0), ("apdt", str(datetime.strptime('1900-01-01', '%Y-%m-%d').date())), ("scdt", str(datetime.strptime('1900-01-01', '%Y-%m-%d').date())), ("pred", 0)))])
...
def submit():
    ...
    finop_data_path = src_path + '/static/json/finalop.json'
    finop1_data_path = src_path + '/Data/Output/finalop1.json'
    patname = request.values.get("Name")
    ...
    global op
    op = np.append(op, np.array([dict((("pt", patname), ("Confirmed", confirmed), ("dept",dept), ("age", int(age)), ("sms", int(sms)), ("apdt", str(appdt)), ("scdt", str(schdt)), ("pred", prediction)))]))
    oplist = op.tolist()
    oplist.pop(0)
    print(oplist)
    ...
    return render_template('appointment.html', prediction_text = 'Appointment Chance {} %'.format(prediction))
